...
export type GetReadExceptionMeterColumsProps = {
  refetch: () => void;
}
export function getReadExceptionMeterColumns({ refetch }: GetReadExceptionMeterColumsProps) {
  const unreadMeterColumns: ColumnDef<Meter>[] = [
      {
      ...
      // },
      {
        accessorKey: "previousKwhReading",
        header: "Previous Reading",
        cell: ({ row }) => row.original.previousKwhReading ? row.original.previousKwhReading + 'kwh' : 0,
      },
      {
        accessorKey: "currentKwhReading",
        header: "Current Reading",
        cell: ({ row }) => row.original.currentKwhReading ? row.original.currentKwhReading + 'kwh' : 0,
      },
      // {
